import re
"""
rio_erp_api.py — RIO PRINT MEDIA ERP v2.0
...
HTML_FILE  = os.environ.get("HTML_FILE", "RIO_PRINT_MEDIA_ERP.html")
...
def ensure_default_users():
    ...
    try:
        ...
        if not existing:
            ...
            if update_fields:
                col("rio_users").update_one({"username": "admin"}, {"$set": update_fields})
    except Exception as e:
...
async def rename_size(request: Request):
    ...
    results = {}
    # Sales records: Size1 and Size2 fields
    ...
    results["sales"] = r1.modified_count + r2.modified_count
...
async def sync_invoice_from_sales(inv_id: int):
    ...
    cust_doc = col("rio_clients").find_one({"ClientName": sale.get("Customer", "")}, {"_id": 0})
    if cust_doc:
